...
void setup_DW1000RSTnIRQ(int enable)
{
	...
	if(enable)
	{
		...
		HAL_NVIC_EnableIRQ(EXTI4_IRQn);		//pin #0 -> EXTI #0 THIHA
		HAL_NVIC_SetPriority(EXTI4_IRQn, 5, 0); //THIHA
	}
	else
	{
		HAL_NVIC_DisableIRQ(EXTI4_IRQn);	//pin #0 -> EXTI #0 THIHA
 ...
 * @brief	set 2.25MHz
 * 			note: hspi1 is clocked from 72MHz (SPI_BAUDRATEPRESCALER_32)
 * @brief	set 2.25MHz
 * 			note: hspi1 is clocked from 144MHz (SPI_BAUDRATEPRESCALER_64)
 * */
 ...
 * @brief	set 18MHz
 * 			note: hspi1 is clocked from 72MHz (SPI_BAUDRATEPRESCALER_4)
 * @brief	set 18MHz
 * 			note: hspi1 is clocked from 144MHz (SPI_BAUDRATEPRESCALER_16)
 * */
...
{
	NVIC_EnableIRQ(DECAIRQ_EXTI_IRQn); //THIHA
}
