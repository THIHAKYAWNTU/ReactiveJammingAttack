...
uint32_t max_TP = 36;
uint16_t JammingDetected = 0; // 0 is not detected, 1 is detected
...
void TXThread (void const *argument) {
  ...
  while (1) {
		...
		if(txSettings.TXEN==1)
		{
			...
			transmit_ack();
			//Results
			sprintf(cmd_Buffer,"PER:  %d, N:  %d\n\r", tx_frame_retry_nb, txSettings.NPackets);
...
//				osSignalSet(tid_uartThread, cmd_Thread);
		...
		else
		{
			...
			tx_frame_retry_nb++;
			if(tx_frame_retry_nb > txSettings.NPackets)
			{
				JammingDetected = 1;
			}
