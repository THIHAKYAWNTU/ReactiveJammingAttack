...
{
  static uint16_t local_buff_offset = 0;

  if( (local_buff_offset+Len) < sizeof(local_buff))
  {
			...
			if(local_buff_length == 5)
			{
				...
				if((local_buff[0] == 100) && (result == 0)) //d (from "deca")
				{
					if(local_buff[4] == 63)
					{
						...
						result = 2;
						led_off(LED_ALL);
						led_on(LED_2); //turn on LED to indicate connection to PC application
					}
				}
			}
		}
...
//	memset(local_buff, 0, sizeof(local_buff));
    ...
    devID = dwt_readdevid() ;
		...
		sprintf((char *)data_to_send, "Device ID: %08X\n\r", (unsigned int)devID); //Convert devID to string
    ...
    if(DWT_DEVICE_ID != devID)
    {
		...
		{
			CDC_Transmit_FS(data_to_send, sizeof(data_to_send));
		}
...
void usb_run(void)
{
    ...
    // loop forever, doing nothing except giving a bit of a LED flash every so often
    {
		...
		{
    		i++ ;
			if (i == 0x0D0000) led_on(LED_3); 
			if (i == 0x0E0000) led_off(LED_3);
			if (i == 0x0F0000) led_on(LED_3);
			if (i == 0x100000)
			{
				led_off(LED_3);
				i = 0;
